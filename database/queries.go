...
const (
	...
	getAllActiveFundsWithUserAndDonationsInfo = "SELECT funds.name, funds.amount, sum(donation.amount) as amount_raised, users.email_id as raised_by from funds left join donation on funds.fund_id = donation.donated_in_fund_id inner join users on funds.raised_by_user_id = users.user_id where funds.status='IN_PROGRESS' group by funds.name,funds.amount,users.email_id"
